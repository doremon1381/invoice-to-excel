...
export default function HomeScreen() {
  ...
  const readyInvoices = useMemo(() => invoices.filter((invoice) => invoice.status === 'success'), [invoices]);
  ...
  const totalAmount = useMemo(
    () => readyInvoices.reduce((sum, invoice) => sum + (invoice.total_amount ?? 0), 0),
    [readyInvoices],
  );
  ...
  return (
    <ThemedView className="flex-1" style={{ backgroundColor: colors.background }}>
      <View className="flex-1 px-5 pb-28 pt-4">
        <View className="mb-5 gap-4">
          <View className="gap-1">
            ...
            <ThemedText style={{ color: colors.muted }}>
              Track extracted invoices, scan new receipts, and export your records with one tap.
            </ThemedText>
          </View>
          ...
          <View className="flex-row gap-3">
            <View
              ...
              <View
                ...
                <View className="flex-row items-center justify-between">
                  <View className="h-20 w-20 items-center justify-center rounded-full border-[10px]" style={{ borderColor: colors.tint }}>
                    <ThemedText type="subtitle">{invoices.length}</ThemedText>
                  </View>
                  <View className="flex-1 pl-4">
                    <ThemedText style={{ color: colors.muted, fontSize: 12, fontWeight: '700' }}>OCR ARCHIVE</ThemedText>
                    ...
                    <ThemedText style={{ color: colors.muted, fontSize: 13 }}>
                      {Math.max(invoices.length - readyInvoices.length, 0)} pending review
                    </ThemedText>
                  </View>
                </View>
              </View>
              <ThemedText type="defaultSemiBold">Expense</ThemedText>
              <ThemedText style={{ color: colors.muted, fontSize: 13 }}>{readyInvoices.length} ready to export</ThemedText>
            </View>
            ...
            <Pressable
              ...
              <View className="gap-2">
                <ThemedText style={{ color: colors.background, fontSize: 20, fontWeight: '700' }}>SCAN NEW</ThemedText>
                <ThemedText style={{ color: colors.background, fontWeight: '700' }}>INVOICE</ThemedText>
                <ThemedText style={{ color: colors.background, fontSize: 12, opacity: 0.9 }}>
                  Send photos to your PaddleOCR Docker workspace.
                </ThemedText>
              </View>
            </Pressable>
          </View>
          ...
          <View className="rounded-[24px] border p-4" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
            <View className="flex-row items-center justify-between gap-4">
              <View>
                <ThemedText style={{ color: colors.muted, fontSize: 12, fontWeight: '600' }}>Ready to export</ThemedText>
                <ThemedText type="subtitle">{readyInvoices.length} invoices</ThemedText>
              </View>
              <View className="items-end">
                <ThemedText style={{ color: colors.muted, fontSize: 12, fontWeight: '600' }}>Tracked total</ThemedText>
        ...
        <View className="mb-4 flex-row rounded-full border p-1" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
        ...
        {isLoading ? (
          ...
          <View className="flex-1">
            <EmptyState
              title={filter === 'all' ? 'No invoices yet' : 'No export-ready invoices'}
              description={
                filter === 'all'
